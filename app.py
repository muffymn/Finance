...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            return apology("symbol not inputed")
        ...
        if not shares or shares < 1:
            return apology("shares < 1 or not inputed")

        transaction_value = float(shares * look["price"])
        user_id = session["user_id"]
        cash_db = db.execute("SELECT cash FROM users WHERE id = ?", user_id)
        cash = cash_db[0]["cash"]
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date) VALUES(?, ?, ?, ?, ?)", user_id, symbol, shares, look["price"], date)
...
def register():
    ...
    if request.method == "POST":
        ...
        if not username or username in users:
            return apology("blank username or username does not exist", 403)
        elif not password or not confirmation or confirmation != password:
            return apology("password and confirmation not matching or blank", 403)
        else:
...
def sell():
    ...
    if request.method == "GET":
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date) VALUES(?, ?, ?, ?, ?)", user_id, symbol, (-1)*shares, look["price"], date)
